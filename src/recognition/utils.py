...
def correct_ocr_confusions(text: str, plate_format: str = r'^\d{2}[A-Z]{3}\d{3}$') -> str:
    ...
    DIGIT_CONFUSIONS = {
        'O': '0', 'Q': '0',  # O and Q look like 0
        'I': '1', 'L': '1',  # I and L look like 1
        'S': '5',            # S looks like 5
        'B': '8',            # B looks like 8
        'Z': '2',            # Z looks like 2
        'G': '6'             # G looks like 6
        ...
        if expected_type == 'digit':
            ...
            if char in DIGIT_CONFUSIONS:
                ...
                if corrected_char != char:
                    logger.debug(f"Position {idx}: corrected '{char}' → '{corrected_char}' (expected digit)")
            ...
            if char in LETTER_CONFUSIONS:
                ...
                if corrected_char != char:
                    logger.debug(f"Position {idx}: corrected '{char}' → '{corrected_char}' (expected letter)")
                corrected.append(corrected_char)
            else:
                corrected.append(char)

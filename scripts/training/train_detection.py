...
Usage:
    ...
    python scripts/training/train_detection.py --config configs/pipeline_config.yaml --data datasets/lpr/data.yaml  # noqa: E501
...
def parse_arguments():
    ...
    parser.add_argument(
        "--device", type=str, default=None, help="Device to use (cuda/cpu/auto, overrides config)"
    )

    parser.add_argument(
        "--patience", type=int, default=10, help="Early stopping patience epochs (default: 10)"
    )
    ...
    parser.add_argument(
        "--name", type=str, default="gta_v_lpr", help="Experiment name (default: gta_v_lpr)"
    )

    parser.add_argument(
        "--exist-ok", action="store_true", help="Allow overwriting existing experiment"
    )
    ...
    parser.add_argument("--save-plots", action="store_true", help="Save training metric plots")
...
def save_training_metrics(
    ...
    try:
        ...
        if test_metrics:
            logger.info(
                f"  Included test metrics: Precision={
                    test_metrics['precision']:.4f}, Recall={
                    test_metrics['recall']:.4f}, mAP50={
                    test_metrics['mAP50']:.4f}"
            )
...
def plot_training_metrics(metrics_csv: Path, output_dir: Path):
    ...
    try:
        ...
        fig.suptitle(
            "Training Metrics - GTA V License Plate Detection", fontsize=16, fontweight="bold"
        )
        ...
        if "mAP50" in df.columns and "mAP50_95" in df.columns:
            axes[1, 1].plot(
                df["epoch"], df["mAP50"], label="mAP@0.5", linewidth=2, marker="o", markersize=3
            )
            axes[1, 1].plot(
                df["epoch"],
                df["mAP50_95"],
        ...
        if "test_mAP50" in df.columns and "test_mAP50_95" in df.columns:
            ...
            if not test_data.empty:
                ...
                ax.set_title(
                    "Mean Average Precision (Validation + Test)", fontsize=14, fontweight="bold"
                )
                ax.legend(fontsize=11)
                ax.grid(True, alpha=0.3)
                ax.set_ylim([0, 1])
...
def train_model(args, config: Dict):
    ...
    try:
        ...
        Path(args.plots_dir) if args.plots_dir else output_dir / "plots"
        ...
        df_metrics = save_training_metrics(
            results_csv, actual_metrics_csv, existing_history, test_metrics
        )

...
Usage:
    ...
    python scripts/inference/detect_image.py --image outputs/test_images/day_clear_front_00001.jpg --output result.jpg  # noqa: E501
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Run license plate detection on a single image")
    ...
    parser.add_argument(
        "--con", type=float, default=None, help="Confidence threshold (overrides config file)"
    )

    parser.add_argument(
        "--iou", type=float, default=None, help="IOU threshold for NMS (overrides config file)"
    )
    ...
    parser.add_argument(
        "--show", action="store_true", help="Display result in window (requires GUI)"
    )
...
def main():
    ...
    try:
        model = load_detection_model(detection_config.model_path, device=detection_config.device)
    except Exception as e:
    ...
    if detections:
        ...
        logger.info(
            f"Try lowering the confidence threshold (current: {
    ...
    if not args.no_save_viz:
        ...
        annotated = draw_bounding_boxes(
            frame, detections, color=(0, 255, 0), thickness=2, show_confidence=True  # Green
        )

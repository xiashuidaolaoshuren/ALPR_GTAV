...
Usage:
    python scripts/convert_labelstudio_to_yolo.py --input export.json --output datasets/lpr
...
def convert_labelstudio_to_yolo(annotation: Dict, image_width: int, image_height: int) -> List[str]:
    ...
    for result in annotation.get('value', {}).get('rectanglelabels', []):
        # Get bounding box (Label Studio format: x, y, width, height in percentages)
        x_percent = annotation['value']['x']  # Top-left x
        y_percent = annotation['value']['y']  # Top-left y
    ...
    for task in tasks:
        ...
        for result in annotation_data.get('result', []):
            if result.get('type') == 'rectanglelabels':
                ...
                # Label Studio format: x, y, width, height (percentages)
                x_percent = value['x']
                y_percent = value['y']
        ...
        # Copy or note image (actual copying would need source path)

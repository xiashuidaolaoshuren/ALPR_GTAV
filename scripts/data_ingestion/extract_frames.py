...
Usage:
    Single video:
        python scripts/data_ingestion/extract_frames.py --input video.mp4 --output datasets/lpr/train/images/  # noqa: E501

    Batch processing:
        python scripts/data_ingestion/extract_frames.py --batch --input_dir raw_footage/ --output_dir datasets/lpr/train/images/  # noqa: E501

    Custom frame rate:
        python scripts/data_ingestion/extract_frames.py --input video.mp4 --output datasets/lpr/train/images/ --fps 10  # noqa: E501
...
class FrameExtractor:
    ...
    def __init__(self, output_fps: int = 5, quality: int = 95, format: str = "jpg"):
        ...
        logger.info(
            f"FrameExtractor initialized: {output_fps} FPS, Quality: {quality}%, Format: {format}"
        )

    def extract_from_video(
        self, video_path: str, output_dir: str, prefix: Optional[str] = None
    ) -> int:
        """
        ...
        logger.info(
            f"  Source FPS: {
                source_fps:.2f}, Total frames: {total_frames}, Duration: {
                duration:.2f}s"
        )
    ...
    def batch_extract(
        ...
        if not video_files:
            logger.warning(
                f"No video files found in {input_dir} with extensions {video_extensions}"
            )
            return {}
...
Examples:
  ...
  python scripts/data_ingestion/extract_frames.py --input footage/day_clear_01.mp4 --output datasets/lpr/train/images/  # noqa: E501

  # Extract from single video at 10 FPS with custom quality
  python scripts/data_ingestion/extract_frames.py --input footage/night.mp4 --output datasets/lpr/valid/images/ --fps 10 --quality 90  # noqa: E501
  ...
  python scripts/data_ingestion/extract_frames.py --input video.mp4 --output images/ --format png
    ...
    parser.add_argument(
        "--input_dir", type=str, help="Path to directory containing videos (for batch mode)"
    )
    ...
    parser.add_argument(
        "--quality", type=int, default=95, help="JPEG quality percentage (0-100, default: 95)"
    )
    parser.add_argument(
        "--format",
        type=str,
    ...
    try:
        if args.batch:
            # Batch mode
            extractor.batch_extract(args.input_dir, args.output)
        else:
            # Single file mode

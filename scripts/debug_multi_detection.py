...
def main():
    ...
    for img_path in test_images:
        ...
        for i, (x1, y1, x2, y2, conf) in enumerate(detections, 1):
            ...
            cv2.putText(
                image, f"Det{i}: {conf:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2
            )
        ...
        if len(detections) >= 2:
            ...
            for i in range(len(detections)):
                for j in range(i + 1, len(detections)):
                    ...
                    # Check overlap
                    if iou > 0.3:
                        print("    → HIGH OVERLAP (should be filtered by NMS)")
                    elif iou > 0.1:
                        print("    → MEDIUM OVERLAP")
                    else:
                        print("    → LOW/NO OVERLAP (different objects)")

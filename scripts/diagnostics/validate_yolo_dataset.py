...
class YOLODatasetValidator:
    ...
    def validate_split(self, split_name: str) -> Dict:
        ...
        if missing_labels:
            self.issues.append(f"{split_name}: {len(missing_labels)} images without labels")
            logger.warning(f"  Missing labels for: {list(missing_labels)[:5]}...")

        if missing_images:
            self.warnings.append(f"{split_name}: {len(missing_images)} labels without images")
        ...
        if not invalid_labels:
            logger.info("  ✓ All label files valid YOLO format")
...
def parse_arguments():
    ...
    parser.add_argument(
        "--no-visualize", action="store_true", help="Skip visualization of sample images"
    )
...
def main():
    ...
    validator.validate(visualize=not args.no_visualize, num_samples=args.samples)

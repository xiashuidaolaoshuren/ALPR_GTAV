...
class PipelineEvaluator:
    ...
    def __init__(self, config_path: str, output_dir: str):
        ...
        # Metrics storage
        self.video_results = {}
        self.condition_results = defaultdict(lambda: {
            'videos': [],
            'total_frames': 0,
            'total_detections': 0,
            'total_tracks': 0,
    ...
    def evaluate_video(
            ...
            for frame_idx, frame in reader.read_frames(sample_rate=1):
                ...
                if save_samples and detections_this_frame > 0:
                    if len(metrics['sample_frames']) < 3:
                        ...
                        annotated = self._annotate_frame(frame.copy(), tracks)
                        metrics['sample_frames'].insert(1, {
                            'frame_idx': frame_idx,
                            'image': annotated,
                            'num_detections': detections_this_frame
                        })
        ...
        metrics['recognition_attempts'] = sum(
            1 for track in self.pipeline.tracks.values() 
        ...
        # Calculate averages
        metrics['avg_processing_time_ms'] = np.mean(metrics['processing_times']) if metrics['processing_times'] else 0
        metrics['avg_confidence'] = np.mean(metrics['confidence_scores']) if metrics['confidence_scores'] else 0
        metrics['avg_detections_per_frame'] = np.mean(metrics['detection_counts_per_frame']) if metrics['detection_counts_per_frame'] else 0
        metrics['avg_track_age'] = np.mean(metrics['track_ages']) if metrics['track_ages'] else 0
        metrics['fps'] = 1000 / metrics['avg_processing_time_ms'] if metrics['avg_processing_time_ms'] > 0 else 0
    ...
    def evaluate_multiple_videos(
        ...
        for condition in self.condition_results:
            ...
            data['avg_detections_per_frame'] = data['total_detections'] / data['total_frames'] if data['total_frames'] > 0 else 0
            data['avg_tracks_per_video'] = data['total_tracks'] / len(data['videos']) if data['videos'] else 0
            data['avg_recognized_per_video'] = data['total_recognized'] / len(data['videos']) if data['videos'] else 0
    ...
    def generate_report(self, results: Dict) -> str:
        ...
        total_recognized = sum(v['unique_plates_recognized'] for v in results['video_results'].values())
        avg_fps = np.mean([v['fps'] for v in results['video_results'].values()])
        avg_confidence = np.mean([v['avg_confidence'] for v in results['video_results'].values() if v['avg_confidence'] > 0])
        ...
        report.append("| Condition | Videos | Frames | Detections | Tracks | Recognized | Avg Confidence |\n")
        report.append("|-----------|--------|--------|------------|--------|------------|----------------|\n")
        ...
        report.append("| Video | Condition | Frames | Detections | Tracks | Recognized | FPS | Avg Conf |\n")
        report.append("|-------|-----------|--------|------------|--------|------------|-----|----------|\n")
        ...
        videos_with_recognition = {
            k: v for k, v in results['video_results'].items()
            if v['unique_plates_recognized'] > 0
        }
        
        report.append(f"- **Videos with Successful Recognition:** {len(videos_with_recognition)}/{len(results['video_results'])}\n")
        
        if videos_with_recognition:
            videos_by_confidence = sorted(
                videos_with_recognition.items(),
        ...
        for condition, data in results['condition_results'].items():
            if data['total_frames'] > 0:
                ...
                recognition_rate = data['total_recognized'] / len(data['videos']) if data['videos'] else 0
        ...
        if condition_performance:
            ...
            report.append(f"Lowest detection rate ({weakest[1]['detection_rate']:.2f} detections/frame).\n")
            report.append("   - Consider augmenting training data for this scenario\n")
            report.append("   - Review detection threshold settings\n\n")
        
        if avg_confidence < 0.7:
            report.append(f"2. **Improve OCR accuracy:** Average confidence is {avg_confidence:.2%}.\n")
...
def main():
    ...
    parser.add_argument(
        '--no-samples',
        action='store_true',
        help='Do not save sample annotated frames'
    )

...
Examples:
    ...
    python scripts/evaluation/evaluate_detection.py --config configs/pipeline_config.yaml --output outputs/results.json  # noqa: E501
"""
...
def _normalize_detections(
    detections: Iterable[Union[DetectionTuple, DetectionDict]],
) -> List[DetectionDict]:
    ...
    for det in detections:
        if isinstance(det, dict):
            ...
            conf = det.get("confidence") or det.get("con")
            if bbox is None or conf is None:
                logger.debug(f"Skipping malformed detection dict: {det}")
                continue
            normalized.append({"bbox": [float(b) for b in bbox], "confidence": float(conf)})
        else:
            ...
            normalized.append(
                {"bbox": [float(x1), float(y1), float(x2), float(y2)], "confidence": float(conf)}
            )
    return normalized
...
def evaluate_detection(
    test_dir: str, model, confidence_threshold: float, iou_threshold: float
) -> Dict:
    ...
    return {"by_condition": dict(results), "all_images": all_results}
...
def calculate_statistics(results: Dict) -> Dict:
    ...
    overall_stats = {
        ...
        "avg_confidence": (
            sum(r["avg_confidence"] for r in all_images) / total_images if total_images else 0
        ),
    }
    ...
    for condition, images in by_condition.items():
        ...
        condition_stats[condition] = {
            ...
            "avg_confidence": (
                sum(r["avg_confidence"] for r in images) / num_images if num_images else 0
            ),
        }

    return {"overall": overall_stats, "by_condition": condition_stats}


def save_examples(results: Dict, test_dir: str, examples_dir: str, num_examples: int = 10):

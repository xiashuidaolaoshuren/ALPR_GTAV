...
def parse_arguments():
    ...
    parser.add_argument(
        '--visualize',
        action='store_true',
        help='Show sample images with bounding boxes'
    )
...
def print_stats(split_name: str, stats: Dict):
    ...
    logger.info(f"Average boxes per image: {stats['total_boxes'] / max(stats['num_images'], 1):.2f}")
...
def main():
    ...
    logger.info(f"Empty labels: {total_stats['empty']} ({total_stats['empty']/max(total_stats['labels'], 1)*100:.1f}%)")
    logger.info(f"Average boxes per image: {total_stats['boxes'] / max(total_stats['images'], 1):.2f}")
